  ...
  begin
    ...
    begin
      ...
      begin
        ...
        SnoozeTime := Now + EncodeTime(0, 5, 0, 0);
        TimerSnooze.Interval := 300000;
...
procedure TAlerm.TimerSnoozeTimer(Sender: TObject);
begin
  if Now <= SnoozeTime then
  begin
    PlaySound('signal-elektronnyih-chasov-budilnik-33058.wav', 0, SND_FILENAME or SND_ASYNC);
    ShowMessage('Время второго сигнала!');
    TimerSnooze.Enabled := False; // Отключение таймера отложенного сигнала
  end;
end;
